...
class Calculator_Custody_Charges_Method_5 {
	...
	public function is_excluded_due_top_aua() {

		$funds_tiers = $this->combine_tiers( 'fund', $this->total_all_assets );
		$ex_instruments_tiers = $this->combine_tiers( 'ex_traded', $this->total_all_assets );
		end($funds_tiers);
		end($ex_instruments_tiers);
		$last_funds_tier_key = key($funds_tiers);
		$last_ex_instrument_tier_key = key($ex_instruments_tiers);

        $total_all_assets = intval( ctp_clean_number( $this->total_all_assets ) );
        $funds_max_aua = intval( ctp_clean_number( $this->platform_data_funds[$last_funds_tier_key]['aua_to'] ));
        $ex_instruments_max_aua = intval( ctp_clean_number( $this->platform_data_ex_instruments[$last_ex_instrument_tier_key]['aua_to'] ));
	...
	public function set_platform_data($platform_data) {
		...
		$this->excluded_due_top_aua = $this->is_excluded_due_top_aua();
	}
		...
		if( $this->funds_tiered ) {
			...
			foreach ( $products as $product ) {
				...
				foreach ($funds_tiers as $tier_key) {
					...
					if ( $this->total_all_assets > $aua_to ) {

						if ( $this->total_all_assets <= $aua_from ) {

							$product_amount = $this->total_all_assets;
						...
						if ( $this->total_all_assets < $aua_to ) {
							$product_amount = $this->total_all_assets - $aua_from;
						}
					}
				...
				if ( $total_product > 0 ) {
					...
					$rate = ( $total_product / $this->total_all_assets ) * 100;
					$this->rates[$product] = $rate;
					$total_product = ( $this->user_data['funds_' . $product] / 100 ) * $this->rates[$product];
				}
	...
	public function total_ex_instruments() {
		...
		if( $this->ex_instruments_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$ex_instruments_tier = $this->combine_tiers( 'ex_traded', $this->total_all_assets );
				...
				if ( $total_product > 0 ) {
					$total_product = round( $total_product, 2 );
					$rate = ( $total_product / $this->total_all_assets ) * 100;
					if (!isset($this->rate[$product])) {
						$this->rates[$product] = $rate;
					}
					$total_product = ( $this->user_data['ex_instruments_' . $product] / 100 ) * $this->rates[$product];
				}
