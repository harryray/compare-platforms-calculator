...
class Calculator_Custody_Charges_Method_3 {
	...
	public function set_platform_data($platform_data) {
		...
		foreach ($this->custody_fees as $key => $value) {
			if ( $value['inv_type'] === 'fund' ) {
				$this->platform_data_funds[] = $value;
			}
			if ( $value['inv_type'] === 'ex_traded' ) {
	...
	public function total_funds() {
		...
		$funds_total_assets = $this->user_data['funds_gia'];
		$funds_total_assets += $this->user_data['funds_isa'];
		$funds_total_assets += $this->user_data['funds_jisa'];
		$funds_total_assets += $this->user_data['funds_sipp'];
		$funds_total_assets += $this->user_data['funds_jsipp'];
		...
		if( $this->funds_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$total_both_products = 'total_'.$product;
				$funds_tiers = $this->combine_tiers( 'fund', $this->$total_both_products);
				...
				foreach ($funds_tiers as $tier_key) {
					...
					if ( $this->$total_both_products > $aua_to ) {

						if ( $this->$total_both_products <= $aua_from ) {

							$product_amount = $this->$total_both_products;
						...
						if ( $this->$total_both_products <= $aua_to ) {
							$product_amount = $this->$total_both_products - $aua_from;
						}
					}
				...
				if ( $total_product > 0 ) {
					if (!isset($this->rate[$product])) {
						$rate = ( $total_product / $this->$total_both_products ) * 100;
	...
	public function total_ex_instruments() {
		...
		$ex_traded_total_assets = $this->user_data['ex_instruments_gia'];
		...
		if( $this->ex_instruments_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$total_both_products = 'total_'.$product;
				$ex_instruments_tier = $this->combine_tiers( 'ex_traded',$this->$total_both_products);
				...
				if ( $total_product > 0 ) {
					...
					$total_product = ( $this->user_data['ex_instruments_' . $product] / 100 ) * $this->rate[$product];
				}
