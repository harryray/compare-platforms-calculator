...
class Calculator_Custody_Charges {
	...
	public function total_funds() {
		...
		if( $this->funds_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$funds_tier = $this->which_tier( 'fund', $this->user_data['funds_'.$product] );
	...
	public function total_ex_instruments() {
		...
		if( $this->ex_instruments_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$ex_instruments_tier = $this->which_tier( 'ex_traded', $this->user_data['ex_instruments_'.$product] );
