...
(function ($) {
    $.fn.repeatable_fields = function (custom_settings) {
        ...
        function before_remove(container, old_row) {

            var del_val = $(old_row).siblings('.deleted').val();
            var old_row_val = $(old_row).find('.fee-id').val();
            del_val = del_val + old_row_val + '|'
            $(old_row).siblings('.deleted').val(del_val);
        }
    };
})(jQuery);
