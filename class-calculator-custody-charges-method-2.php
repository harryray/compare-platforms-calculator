...
class Calculator_Custody_Charges_Method_2 {
	...
	public function total_funds() {
		...
		if( $this->funds_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$funds_tiers = $this->combine_tiers( 'fund', $this->user_data['funds_'.$product] );

				//TODO Split product assets per tier
				foreach ($funds_tiers as $tier_key) {
					$users_product_amount = (int) $this->user_data['funds_' . $product];
	...
	public function total_ex_instruments() {
		...
		if( $this->ex_instruments_tiered ) {
			...
			foreach ( $products as $product ) {
				$total_product = 0;
				$ex_instruments_tier = $this->combine_tiers( 'ex_traded', $this->user_data['ex_instruments_'.$product] );

				// @TODO Split product assets per tier
				foreach ($ex_instruments_tier as $tier_key) {

					$users_product_amount = (int) $this->user_data['ex_instruments_' . $product];
